 ...
 *
 * The BIP-322 verifier pulls in @scure/btc-signer and @noble/*, so it is
 * imported on demand rather than bundled into first paint: nothing here is on
 * a critical path (the badge resolves a moment after connect) and most visitors
 * never connect a wallet at all.
 */
async function checkProof(proof: ConnectionProof, addr: string): Promise<ProofStatus> {
  const { canVerifyBip322, verifyBip322 } = await import('@/lib/bip322')
...
export function WalletProvider({ children }: { children: ReactNode }) {
  ...
  /**
   * The active address's public key, resolved whenever the address changes.
   *
   ...
   * never has, so without this its first transaction of that shape fails on
   ...
   *     the internal key. The wallet defaults new accounts to taproot.
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (match) {
        setKeyedPublicKey({ address, publicKey: match.publicKey })
        return
      }
      if (connectionProof?.address !== address) return
      const { pubkeyFromBip322 } = await import('@/lib/bip322')
      const recovered = pubkeyFromBip322(address, connectionProof.signature)
      if (cancelled || !recovered) return
      setKeyedPublicKey({ address, publicKey: recovered })
    })()
    return () => {
      cancelled = true
    }
  ...
  useEffect(() => {
    ...
    const onAccountsChanged = (accounts: string[]) => {
      ...
      if (accounts.length === 0) {
        ...
        // the address on unlock; the connection is retained. Revocation
        // arrives as 'disconnect'. Stay connected — a signing attempt on a
        // locked wallet opens its unlock prompt anyway.
        return
      }
  ...
  const connect = async () => {
    ...
    try {
      ...
      if (result && result.accounts.length > 0) {
        ...
        // security boundary — anything that actually matters (the swap listing
        // flow) re-verifies server-side, and that is the real gate. What it
        // buys is an honest answer about what we know, and an address type we
        // can't check is a coverage gap, not a red flag, so it must not be
        // reported the same way as a signature that didn't verify.
