...
export default function TradePageLayoutC() {
  const [tradeTab, setTradeTab] = useState<'buy' | 'sell'>('buy')
  const [dataTab, setDataTab] = useState<'trades' | 'holders'>('trades')
  const [mobileDataTab, setMobileDataTab] = useState<'book' | 'trades' | 'holders'>('book')
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans">
      ...
      <div className="border-b border-zinc-800 bg-zinc-950">
        ...
        {/* Chart visualization -- taller full-width */}
        <div className="relative px-4 py-4 max-sm:px-2" style={{ minHeight: '340px' }}>
          {/* Mobile: slightly shorter */}
          <style>{`@media (max-width: 1023px) { .chart-area-c { min-height: 200px !important; } }`}</style>
          <div className="chart-area-c absolute inset-0" />
      ...
      {/* -- Mobile Data Tab Bar --------------------------------------------- */}
      <div className="lg:hidden bg-zinc-950 flex border-b border-zinc-800">
        {(['book', 'trades', 'holders'] as const).map((tab) => (
          <button
            key={tab}
          ...
          >
            {tab === 'book' ? 'Order Book' : tab === 'trades' ? 'Trades' : 'Holders'}
          </button>
        ))}
      </div>

      {/* -- Mobile: Trade Form (below chart, above tabs content) ------------ */}
      <div className="lg:hidden bg-zinc-950 border-b border-zinc-800 p-3">
        {/* Buy/Sell toggle */}
              ...
              {holders.map((h, i) => (
                ...
                >
                  <span className="text-zinc-300 font-mono">
                    {h.address}
                    {h.tag && (
                      <span className="ml-1 text-[10px] text-zinc-600">({h.tag})</span>
                    )}
                  </span>
                  <span className="text-right text-zinc-400 font-mono">{h.balance}</span>
                  <span className="text-right text-zinc-500 font-mono">{h.pct}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* -- Section 2: Three-column grid (desktop only) --------------------- */}
      <div className="hidden lg:grid lg:grid-cols-12 gap-px bg-zinc-800">

        {/* -- Left: Order Book (col-span-3) --------------------------------- */}
        <div className="lg:col-span-3 bg-zinc-950">
        ...
        {/* -- Center: Buy/Sell Trade Form (col-span-5) ---------------------- */}
        <div className="lg:col-span-5 bg-zinc-950 p-3">
          {/* Buy/Sell toggle */}
          <div className="mb-3 flex rounded-sm overflow-hidden">
            <button
              onClick={() => setTradeTab('buy')}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            {/* Right column: % buttons + Total + Fee */}
            <div className="space-y-2">
              <div className="flex flex-col gap-1">
                {['25%', '50%', '75%', '100%'].map((pct) => (
                  <button
                    key={pct}
                    className="w-full rounded-sm border border-zinc-800 bg-zinc-900 py-1.5 text-xs text-zinc-500 hover:border-zinc-700 hover:text-zinc-300 transition-colors"
                  >
                    {pct}
                  </button>
                ))}
          ...
          <button
            className={`mt-3 w-full rounded-sm py-2.5 text-xs font-semibold uppercase tracking-wider transition-colors ${
              tradeTab === 'buy'
                ? 'bg-green-500 text-zinc-950 hover:bg-green-400'
                : 'bg-red-500 text-zinc-950 hover:bg-red-400'
            }`}
          ...
          {/* Order Summary divider + row */}
          <div className="mt-3 border-t border-zinc-800 pt-2">
            <div className="flex items-center justify-between text-xs">
              <div className="flex items-center gap-1.5">
                <span className="text-zinc-600">Best Bid</span>
                <span className="text-green-400 font-mono">0.00000420</span>
              </div>
              <div className="flex items-center gap-1.5">
                <span className="text-zinc-600">Spread</span>
                <span className="text-zinc-300 font-mono">0.00000010</span>
                <span className="text-zinc-600">(0.24%)</span>
              </div>
              <div className="flex items-center gap-1.5">
                <span className="text-zinc-600">Best Ask</span>
        ...
        {/* -- Right: Trades / Holders tabs (col-span-4) --------------------- */}
        <div className="lg:col-span-4 bg-zinc-950">
          {/* Tab header */}
