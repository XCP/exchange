 ...
 */
export default function BuyClient({ asset: routeAsset }: { asset: ResolvedAsset | null }) {
  ...
  return (
    <Suspense>
      <DispenseSurface
        ...
        mode="buy"
        onModeChange={(m) => m === 'sell' && router.push(pairPath('/sell', segment(selection.canonical)), { scroll: false })}
        onAssetChange={(next, longname) => {
          setSelection({ name: next, canonical: longname ?? next })
          replacePairPath(pairPath('/buy', segment(longname ?? next)))
        }}
      />
    </Suspense>
  )
