...
export default function TradePageLayoutL() {
  ...
  const maxAskAmt = Math.max(...asks.map((a) => parseFloat(a.amount.replace(/,/g, ''))))
  const maxBidAmt = Math.max(...bids.map((b) => parseFloat(b.amount.replace(/,/g, ''))))
  const maxAskTotal = Math.max(...asks.map((a) => parseFloat(a.total)))
  const maxBidTotal = Math.max(...bids.map((b) => parseFloat(b.total)))

  // Top 5 asks sorted so lowest ask is at bottom (nearest the spread)
  const top5Asks = asks.slice(-5)
  // Top 5 bids — highest bid at top (nearest the spread)
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans">
      ...
      {/* -- 2. Full-Width Sticky Market Header -------------------------------- */}
        ...
        <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
          ...
          <div className="hidden md:flex gap-5">
            ...
            <div>
              ...
              <div className="text-xs text-zinc-300 font-mono">0.24%</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-px bg-zinc-800">
        ...
        {/* == Left Column -- Order Book (desktop only, narrow) ================ */}
        <div
          className="hidden lg:flex lg:col-span-2 bg-zinc-950 flex-col overflow-y-auto"
          style={{ maxHeight: 'calc(100vh - 37px)' }}
        >
          {/* Heading */}
          <div className="px-2 py-2 border-b border-zinc-800">
            <span className="text-xs text-zinc-500 uppercase tracking-wider">Order Book</span>
          </div>

          {/* Column headers */}
          <div className="grid grid-cols-2 gap-0 px-2 py-1 text-[10px] text-zinc-600 border-b border-zinc-800">
            <span>Price</span>
            <span className="text-right">Amt</span>
          </div>

          {/* Asks -- top 5, lowest ask at bottom (nearest spread) */}
          <div className="px-0.5">
            {top5Asks.map((ask, i) => {
              const amt = parseFloat(ask.amount.replace(/,/g, ''))
              const depthPct = (amt / maxAskAmt) * 100
              return (
                <div
                  key={`ask-${i}`}
                  className="relative grid grid-cols-2 gap-0 px-2 py-px hover:bg-zinc-900 cursor-pointer"
                >
                  <div
                    className="absolute inset-y-0 right-0 bg-red-500/10"
                    style={{ width: `${depthPct}%` }}
                  />
                  <span className="relative z-10 text-red-400 font-mono text-[10px]">{ask.price}</span>
                  <span className="relative z-10 text-right text-zinc-400 font-mono text-[10px]">{ask.amount}</span>
                </div>
              )
            })}
          </div>
          ...
          {/* Spread */}
          <div className="flex items-center justify-center gap-1.5 border-y border-zinc-800 bg-zinc-900/50 px-2 py-1.5">
          ...
          {/* Bids -- top 5, highest bid at top (nearest spread) */}
          <div className="px-0.5">
            {top5Bids.map((bid, i) => {
              const amt = parseFloat(bid.amount.replace(/,/g, ''))
              const depthPct = (amt / maxBidAmt) * 100
              return (
                <div
                  key={`bid-${i}`}
                  className="relative grid grid-cols-2 gap-0 px-2 py-px hover:bg-zinc-900 cursor-pointer"
                >
                  <div
                    ...
                    style={{ width: `${depthPct}%` }}
                  />
                  <span className="relative z-10 text-green-400 font-mono text-[10px]">{bid.price}</span>
                  <span className="relative z-10 text-right text-zinc-400 font-mono text-[10px]">{bid.amount}</span>
                </div>
              )
            })}
          </div>

          {/* Last trade footer -- pushed to bottom */}
          <div className="mt-auto border-t border-zinc-800 px-2 py-2">
            <div className="flex items-center justify-between">
              <span className="text-[10px] text-zinc-600">Last</span>
              <div className="flex items-center gap-1.5">
                <span className="text-[10px] text-green-400 font-mono">0.00000420</span>
        ...
        <div className="order-1 lg:order-none col-span-1 lg:col-span-7 bg-zinc-950 flex flex-col">
          ...
          {/* -- Desktop Tab Bar (no Order Book tab -- it has its own column) --- */}
        ...
        {/* == Right Column -- Form + Order Summary + Quick Stats ============== */}
        <div className="order-2 lg:order-none col-span-1 lg:col-span-3 bg-zinc-950 flex flex-col">
        ...
        {/* -- Mobile Data Content --------------------------------------------- */}
        <div className="order-4 lg:hidden bg-zinc-950 h-[300px] overflow-y-auto">
