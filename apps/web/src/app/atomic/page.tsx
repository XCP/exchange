    ...
    <table className="w-full whitespace-nowrap text-xs">
      <thead>
        <tr className="border-b border-zinc-800 text-zinc-500">
          ...
          <th className="px-3 py-2 text-right font-normal">{satsMode ? 'Unit (sats)' : 'Unit (BTC)'}</th>
          ...
          {tab === 'filled' && <th className="px-3 py-2 text-left font-normal">Buyer</th>}
          <th className="px-3 py-2 text-right font-normal">{tab === 'filled' ? 'Filled' : 'Listed'}</th>
      ...
      <tbody>
        {isLoading || listings.length === 0 ? (
          ...
          listings.map((listing) => {
            ...
            const unitPriceBtc = fromSats(unitPrice)
            const totalPriceBtc = fromSats(totalSats)
            ...
            return (
              ...
              >
                ...
                <td className="px-3 py-1.5">
                  {isExactMatch ? (
                    <Link href={`/atomic/${listing.asset}`} className="flex items-center gap-1.5 hover:underline">
                      <Image
