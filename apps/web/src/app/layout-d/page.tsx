...
export default function TradePageLayoutD() {
  const [tradeTab, setTradeTab] = useState<'buy' | 'sell'>('buy')
  const [dataTab, setDataTab] = useState<'trades' | 'orders' | 'holders' | 'markets'>('trades')
  const [mobileDataTab, setMobileDataTab] = useState<'book' | 'trades' | 'orders' | 'holders' | 'markets'>('book')
  ...
  // Lock icon SVG for the Orders tab empty state
  const LockIcon = () => (
    <svg
  ...
  // Orders tab content (not connected state)
  const OrdersEmptyState = () => (
    <div className="flex flex-col items-center justify-center h-full gap-3">
      <LockIcon />
      <span className="text-sm text-zinc-500">Connect your wallet to view orders</span>
      <button className="text-xs text-green-400 hover:text-green-300 underline underline-offset-2 transition-colors">
        Connect Wallet
      </button>
    </div>
  )

  {/* Connected state for Orders tab (commented alternative):
  const OrdersConnectedState = () => (
    <div>
      <div className="grid grid-cols-7 gap-0 px-3 py-1.5 text-xs text-zinc-600">
        <span>Order #</span>
        <span>Pair</span>
        <span>Side</span>
      ...
      <div className="px-1">
        {orders.map((order, i) => (
          <div key={`order-${i}`} className="grid grid-cols-7 gap-0 px-2 py-px text-xs hover:bg-zinc-900 cursor-default">
            <span className="text-zinc-300 font-mono">{order.id}</span>
            <span className="text-zinc-300 font-mono">{order.pair}</span>
            <span className={`font-mono ${order.side === 'buy' ? 'text-green-400' : 'text-red-400'}`}>{order.side}</span>
            <span className="text-right text-zinc-400 font-mono">{order.price}</span>
            <span className="text-right text-zinc-400 font-mono">{order.amount}</span>
            <span className="text-right text-zinc-500 font-mono">{order.status}</span>
            <span className="text-right text-zinc-600 font-mono">{order.time}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  */}
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans">
      ...
      {/* -- NO market header strip -- info moved to left sidebar ----------- */}

      {/* -- Main Grid ------------------------------------------------------- */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-px bg-zinc-800">

        {/* -- LEFT COLUMN: Market Info + Trade Form + Order Summary (lg:col-span-3)
             On mobile this appears after the chart (order-2).
             On desktop it's the first column (lg:order-1).
        -------------------------------------------------------------------- */}
        <div className="order-2 lg:order-1 col-span-1 lg:col-span-3 bg-zinc-950 flex flex-col">

          {/* Mobile: condensed market info (pair + price + change on one line) */}
          <div className="lg:hidden px-3 pt-3 pb-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-zinc-300 font-mono">0.00000420</span>
              <span className="rounded-sm bg-green-500/10 px-1.5 py-px text-[10px] font-medium text-green-400">
                +5.2%
              </span>
            </div>
          </div>

          {/* Desktop: Full Market Info Card */}
          <div className="hidden lg:block p-3 pb-0">
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 mb-3">
              <div className="flex items-center justify-between">
                <span className="text-xs text-zinc-500">24h High</span>
                <span className="text-xs text-zinc-300 font-mono">0.00000450</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-zinc-500">24h Low</span>
                <span className="text-xs text-zinc-300 font-mono">0.00000390</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-zinc-500">24h Vol</span>
                <span className="text-xs text-zinc-300 font-mono">3.42 XCP</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-zinc-500">Spread</span>
                <span className="text-xs text-zinc-300 font-mono">2.38%</span>
              </div>
            </div>
        ...
        <div className="order-1 lg:order-2 col-span-1 lg:col-span-6 bg-zinc-950 flex flex-col">
          ...
          {/* Tabbed Data Section (desktop only -- Trades | Orders | Holders | Markets)
              Fixed height container to prevent CLS */}
          <div className="hidden lg:block">
            <div className="flex border-b border-zinc-800">
              {(['trades', 'orders', 'holders', 'markets'] as const).map((tab) => (
                <button
                  key={tab}
                ...
                >
                  {tab === 'trades' ? 'Trades' : tab === 'orders' ? 'Orders' : tab === 'holders' ? 'Holders' : 'Markets'}
                </button>
              ))}
            </div>

            {/* Fixed-height scrollable content area */}
            <div className="h-[320px] overflow-y-auto [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-zinc-950 [&::-webkit-scrollbar-thumb]:bg-zinc-800 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-zinc-700">
              ...
              {/* Orders tab content (not connected) */}
              {dataTab === 'orders' && (
                <OrdersEmptyState />
              )}
        ...
        {/* -- Mobile Data Tab Bar -------------------------------------------
             ...
             orders, holders, and markets.
        -------------------------------------------------------------------- */}
        <div className="order-3 lg:hidden bg-zinc-950 flex border-b border-zinc-800">
          {(['book', 'trades', 'orders', 'holders', 'markets'] as const).map((tab) => (
            <button
              key={tab}
            ...
            >
              {tab === 'book' ? 'Order Book' : tab === 'trades' ? 'Trades' : tab === 'orders' ? 'Orders' : tab === 'holders' ? 'Holders' : 'Markets'}
            </button>
          ))}
        </div>
        ...
        <div className={`order-5 lg:hidden col-span-1 bg-zinc-950 ${
          ...
          <div className="h-[280px] overflow-y-auto [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-zinc-950 [&::-webkit-scrollbar-thumb]:bg-zinc-800 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-zinc-700">
            <div className="grid grid-cols-3 gap-0 px-3 py-1.5 text-xs text-zinc-600">
              <span>Price (XCP)</span>
              <span className="text-right">Amount</span>
              <span className="text-right">Time</span>
        ...
        {/* -- Mobile: Orders content (toggled via mobileDataTab) ------------ */}
        <div className={`order-6 lg:hidden col-span-1 bg-zinc-950 ${
          mobileDataTab !== 'orders' ? 'hidden' : ''
        }`}>
          <div className="h-[280px] overflow-y-auto [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-zinc-950 [&::-webkit-scrollbar-thumb]:bg-zinc-800 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-zinc-700">
            <OrdersEmptyState />
          </div>
        ...
        <div className={`order-8 lg:hidden col-span-1 bg-zinc-950 ${
