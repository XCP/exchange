...
import { YourDispensersPanel } from '@/components/your-orders-panel'
...
import { fromSats, toSats } from '@/utils/numeric'
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      <div className="px-4 py-8">
        <BrowseHeader title="Dispensers" subtitle="BTC-to-asset vending machines on Counterparty">
          <CreateAction href="/sell" label="+ Create dispenser" />
    ...
    <table className="w-full whitespace-nowrap text-xs">
      <thead>
        <tr className="border-b border-zinc-800 text-zinc-500">
          ...
          <th className="px-3 py-2 text-right font-normal">Dispense Cost</th>
            ...
            return (
              <tr key={d.tx_hash} className="hover:bg-zinc-800/50 transition-colors border-b border-zinc-800/30 last:border-0">
                ...
                <td className={`font-medium px-3 py-1.5 text-green-400`}>
                  {isOpen ? (
                    <Link
                      href={`/buy/${encodeURIComponent(d.asset)}?address=${d.source}`}
                ...
                <td className="text-right text-zinc-500 font-mono px-3 py-1.5">
                  {d.give_quantity > 1 ? formatPrice(fromSats(d.satoshi_price), satsMode) : ''}
                </td>
                <td className="text-right text-zinc-500 font-mono px-3 py-1.5">
    ...
    <table className="w-full whitespace-nowrap text-xs">
      <thead>
        <tr className="border-b border-zinc-800 text-zinc-500">
          ...
          <SortHeader label="Effective Price" sortKey="price" currentSort={sort} onSort={onSort} disabled={!hasFilter} className="text-right" />
          <th className="py-1.5" />
          <th className="px-3 py-2 text-right font-normal">Dispensed</th>
          <th className="text-left font-normal px-3 py-0.5 min-w-0">
            <span className="relative flex items-center">
              <input
                type="text"
          ...
          <th className="px-3 py-2 text-right font-normal">Total ({satsMode ? 'sats' : 'BTC'})</th>
          <th className="px-3 py-2 text-left font-normal">
