    ...
    await db
      .prepare(
        ...
        WHERE pair_stats.pair = json_extract(j.value, '$.p')
          AND ( pair_stats.last_price        IS NOT json_extract(j.value, '$.lp')
             OR pair_stats.last_trade_time   IS NOT json_extract(j.value, '$.lt')
             OR pair_stats.last_side         IS NOT json_extract(j.value, '$.ls')
             OR pair_stats.first_trade_time  IS NOT json_extract(j.value, '$.ft')
             OR pair_stats.price_change_24h  IS NOT json_extract(j.value, '$.pc24')
             OR pair_stats.price_change_30d  IS NOT json_extract(j.value, '$.pc30')
             OR pair_stats.price_change_1y   IS NOT json_extract(j.value, '$.pc1y')
             OR pair_stats.volume_24h        IS NOT json_extract(j.value, '$.v24')
             OR pair_stats.volume_30d        IS NOT json_extract(j.value, '$.v30')
             OR pair_stats.volume_1y         IS NOT json_extract(j.value, '$.v1y')
             OR pair_stats.base_volume_24h   IS NOT json_extract(j.value, '$.bv24')
             OR pair_stats.base_volume_30d   IS NOT json_extract(j.value, '$.bv30')
             OR pair_stats.base_volume_1y    IS NOT json_extract(j.value, '$.bv1y')
             OR pair_stats.high_24h          IS NOT json_extract(j.value, '$.h24')
             OR pair_stats.low_24h           IS NOT json_extract(j.value, '$.l24')
             OR pair_stats.high_30d          IS NOT json_extract(j.value, '$.h30')
             OR pair_stats.low_30d           IS NOT json_extract(j.value, '$.l30')
             OR pair_stats.high_1y           IS NOT json_extract(j.value, '$.h1y')
             OR pair_stats.low_1y            IS NOT json_extract(j.value, '$.l1y')
             OR pair_stats.trade_count_24h   IS NOT json_extract(j.value, '$.c24')
             OR pair_stats.trade_count_30d   IS NOT json_extract(j.value, '$.c30')
             OR pair_stats.trade_count_1y    IS NOT json_extract(j.value, '$.c1y')
             OR pair_stats.total_volume      IS NOT json_extract(j.value, '$.tv')
             OR pair_stats.total_base_volume IS NOT json_extract(j.value, '$.tbv')
             OR pair_stats.total_trade_count IS NOT json_extract(j.value, '$.tc')
             OR pair_stats.unique_traders    IS NOT json_extract(j.value, '$.ut')
             OR pair_stats.self_trade_pct    IS NOT json_extract(j.value, '$.stp')
             OR pair_stats.all_time_high     IS NOT json_extract(j.value, '$.ath')
             OR pair_stats.all_time_low      IS NOT json_extract(j.value, '$.atl'))`
      )
