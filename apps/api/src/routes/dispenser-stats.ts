export async function handleDispenserStatsList(
  ...
  const activityFilter = tf === "all" ? ` AND ds.total_dispense_count >= 20` : ` AND ds.dispense_count_${tf} > 0`;
  const activityFilterCount = tf === "all" ? ` AND total_dispense_count >= 20` : ` AND dispense_count_${tf} > 0`;
