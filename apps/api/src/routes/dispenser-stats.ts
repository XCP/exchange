  ...
  const [countResult, rows, summaryResult] = await db.batch([
    ...
    db.prepare(
      `SELECT
         ...
         (SELECT COALESCE(SUM(p.quote_volume), 0) FROM dispenses p${includeHidden ? "" : " WHERE NOT EXISTS (SELECT 1 FROM dispenser_stats s WHERE s.asset = p.asset AND s.hidden = 1)"}) AS total_btc_volume,
