...
import { acknowledgeWebSocketClose } from "../utils/websocket-close";

/**
 * One singleton room for the whole site — how many people currently have
 * xcpdex.com open, anywhere, not per-launch. Traffic here is expected to be low
 * enough that a per-page count would mostly read 0 or 1, which isn't an
 ...
 * the connections themselves, hibernated so idle viewers cost nothing between messages.
 *
 ...
 * markets side by side counted twice. Each client sends an opaque id that is
...
export class SitePresence extends DurableObject<Env> {
  ...
  async webSocketClose(ws: WebSocket, code: number, reason: string) {
    acknowledgeWebSocketClose(ws, code, reason);
    this.broadcastCount();
  }

  async webSocketError(ws: WebSocket) {
    acknowledgeWebSocketClose(ws, 1011, "WebSocket error");
    this.broadcastCount();
  }
